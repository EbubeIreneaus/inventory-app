<template>
  <div>
    <aside
      ...
      :class="{ 'lg:max-w-[20%] md:max-w-[50%] sm:max-w-[70%] max-w-[90%] opacity-100 shadow-md lg:shadow-none shadow-secondary/30': asideOpen }"
    >
       <div class="p-3 lg:hidden">
        <u-button icon="lucide:x" color="error" variant="ghost" class="float-right" @click="asideOpen = false" />
       </div>
      <div class="py-5 mt-5 px-2">
        <div class="flex-1 text-center capitalize  mb-5">
        <h2 class="text-xl font-semibold opacity-40">{{ user?.firstname }} {{ user?.lastname }}</h2>
        <p class="text-sm text-gray-500">{{ user?.role }}</p>
        <p class="text-sm text-gray-500">{{ user?.previlage }}, {{ user?.state }}</p>
      </div>
        <div>
          <u-navigation-menu
      ...
      <header class="bg-surface-light dark:bg-surface-dark">
        <u-container class="flex justify-between items-center py-4">
          ...
          <div class="flex items-center gap-x-3">
            <u-button icon="lucide:moon" variant="ghost" class="dark:hidden" color="secondary" @click="toggleColorMode('dark')" />
            <u-button
              icon="lucide:sun"
              variant="ghost"
